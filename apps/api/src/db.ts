...
export function getSettings(db:DatabaseSync):Settings { const rows=db.prepare("SELECT key,value FROM settings").all() as {key:string,value:string}[]; const values=Object.fromEntries(rows.map(r=>[r.key,r.value])); return {defaultAction:values.defaultAction as Settings["defaultAction"],historyPageSize:Number(values.historyPageSize)}; }
